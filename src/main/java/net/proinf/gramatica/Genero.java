...
public enum Genero {
    ...
    private static class Terminacion {
        ...
        boolean de(Palabra palabra) {
            return terminacion != null && palabra.acabaEn(terminacion);
        }
    }

    private static class Terminaciones implements Iterable<Genero.Terminacion> {
        private final ArrayList<Terminacion> list = new ArrayList<>();

        public Iterator<Genero.Terminacion> iterator() {
            return new TerminacionesIterator();
        }
