...
public class Silabas implements Iterable<Silabas.Silaba>, Observer {
    ...
    private final ArrayList<Silaba> list = new ArrayList<>();
    ...
    public Iterator<Silaba> iterator() {
        return new SilabasIterator();
    }
        ...
        for (Silaba silaba : this) {
            if (silaba.todoConsonantes()
                ...
                if (silaba.numeroLetras() == 1) {
                    ...
                    silaba.juntarAdyacentesPartiendoDesdeFinal(); // tr-a↔ns/gr↔e-s-ió-n
    ...
    public Silaba silaba(int posicion) {
        return list.get(posicion);
    }

    public int numeroSilabas() {
        return list.size();
    }

    public boolean haySilabas() {
        return list.size() > 0;
    }

    public Silaba primeraSilaba() {
        return list.get(0);
    }

    public Silaba segundaSilaba() {
        return list.get(1);
    }

    public Silaba ultimaSilaba() {
        return list.get(list.size() - 1);
    }

    public Silaba penultimaSilaba() {
        return list.get(list.size() - 2);
    }

    public Silaba antepenultimaSilaba() {
    ...
    private void agregarProtosilaba(Silaba silaba) {
        list.add(silaba);
    }

    private void actualizarPalabraReferenciada() {
        palabra.cambiarPor(unirSilabas());
    }
    ...
    public class Silaba extends Letras<Silaba> {
        ...
        public boolean esTonica() {
            return tonica;
        }

        public boolean esAtona() {
            return !tonica;
        }
        ...
        private void marcarAtona() {
            tonica = false;
        }

        private void marcarTonica() {
            tonica = true;
        }
         ...
         */
        private Silaba siguiente() {
            return list.get(list.indexOf(this) + 1);
        }

        private Silaba anterior() {
            return list.get(list.indexOf(this) - 1);
        }
         ...
         */
        private Silaba juntarAdyacentesPartiendoDesdeFinal() {
            return juntarAdyacentesPartiendoDesde(-2);
        }
    }
}
